...
import { LinkAuthUserModal } from "./components/LinkAuthUserModal";
...
export default function App() {
  ...
  const [showLinkAuthModal, setShowLinkAuthModal] = useState(false);
  const [needsAuthLink, setNeedsAuthLink] = useState(false);

  // Check if user needs to link their auth account
  useEffect(() => {
    if (user && !authLoading) {
      getCurrentUserProfile().then((profile) => {
        if (!profile && !needsAuthLink) {
          // User is authenticated but has no profile
          setNeedsAuthLink(true);
          setShowLinkAuthModal(true);
        }
      }).catch(() => {
        // Silent catch
      });
      ...
      {!authLoading && user && (() => {
        ...
        return (
          <>
            ...
            >
              {/* Warning Banner for Unlinked Auth */}
              {needsAuthLink && (
                <div className="bg-red-600 text-white px-6 py-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm">Your authentication account is not linked to a user profile. Click here to link your account.</p>
                    </div>
                  </div>
                  <button
                    onClick={() => setShowLinkAuthModal(true)}
            ...
            {showLinkAuthModal && (
              <LinkAuthUserModal
                onClose={() => setShowLinkAuthModal(false)}
                onLink={() => {
                  setNeedsAuthLink(false);
                  setShowLinkAuthModal(false);
                }}
              />
            )}

...
export function ArticleDetailEdit({ track, onBack, onUpdate, onVersionClick, isSuperAdminAuthenticated = false, isNewContent = false, onNavigateToPlaylist, registerUnsavedChangesCheck, onArchive, onDuplicate, onCreateVariant }: ArticleDetailEditProps) {
    ...
    const handleFactsRegenerated = async (event: CustomEvent) => {
      if (event.detail.trackId === track.id) {
        ...
        try {
          ...
          toast.success(t('articleDetail.keyFactsReloaded', { count: facts.length }));
        } catch (error) {
          console.error('Failed to reload facts:', error);
          toast.error(t('articleDetail.failedToReloadFacts'));
        }
      }
    };
  ...
  const handleSave = async () => {
    ...
    if (!editFormData.title.trim()) {
      toast.error(t('articleDetail.titleRequired'));
      return;
    }
    ...
    try {
      ...
      toast.success(contentChanged ? t('articleDetail.articleUpdated') : t('articleDetail.settingsUpdated'));
      setIsEditMode(false);
          ...
          if (plainText.length >= 150) {
            ...
            if (response.ok) {
              ...
              if (data.skipped) {
                ...
                toast.success(t('articleDetail.autoGeneratedKeyFacts', { count: data.enriched?.length || 0 }));
              }
      ...
      toast.error(error.message || t('articleDetail.failedToSave'));
    } finally {
  ...
  const handleRemoveLearningObjective = async (index: number) => {
    ...
    if (factToRemove?._dbId) {
      try {
        ...
        toast.success(t('articleDetail.keyFactRemoved'));
      } catch (error: any) {
        console.error('Failed to delete fact:', error);
        toast.error(t('articleDetail.failedToRemoveFact'));
  ...
  const handleGenerateKeyFacts = async () => {
    ...
    if (!editFormData.article_body || editFormData.article_body.trim().length < 100) {
      toast.error(t('articleDetail.addContentFirst'));
      return;
    }
    ...
    if (hasExistingFacts) {
      ...
      try {
        ...
        if (newFacts.length === 0) {
          toast.error(t('articleDetail.noKeyFactsGenerated'));
          return;
        }
        ...
        toast.success(t('articleDetail.keyFactsGenerated', { count: newFacts.length }));

      } catch (error: any) {
        console.error('❌ Error generating key facts:', error);
        toast.error(error.message || t('articleDetail.failedToGenerateFacts'));
      } finally {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(t('articleDetail.fileUploaded'));
    } catch (error: any) {
      console.error('Error uploading file:', error);
      toast.error(error.message || t('articleDetail.failedToUploadFile'));
    } finally {
  ...
  const handleAttachmentUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 10485760) {
      toast.error(t('articleDetail.fileTooLarge'));
      e.target.value = '';
      return;
    }
    ...
    try {
      ...
      toast.success(t('articleDetail.attachmentUploaded'));
      ...
      toast.error(error.message || t('articleDetail.failedToUploadAttachment'));
    } finally {
      setIsUploading(false);
    }
  };
  ...
  const handleDeleteAttachment = async (attachmentId: string) => {
    if (!confirm(t('articleDetail.confirmDeleteAttachment'))) {
      return;
    }
    ...
    try {
      ...
      toast.success(t('articleDetail.attachmentDeleted'));
    } catch (error: any) {
      console.error('Error deleting attachment:', error);
      toast.error(error.message || t('articleDetail.failedToDeleteAttachment'));
    }
  };
  ...
  const handleImageUpload = async (file: File): Promise<string> => {
    try {
      const result = await crud.uploadTrackFile(track.id, file);
      toast.success(t('articleDetail.imageUploaded'));
      return result.url;
    } catch (error: any) {
      console.error('Error uploading image:', error);
      toast.error(error.message || t('articleDetail.failedToUploadImage'));
      throw error;
    }
  };
  ...
  const handleKBToggle = async (checked: boolean) => {
    if (isEditMode) {
      ...
      try {
        ...
        toast.success(checked ? t('articleDetail.trackAddedToKb') : t('articleDetail.trackRemovedFromKb'));
        ...
        toast.error(t('articleDetail.failedToUpdateKb'), {
          description: error.message || t('articleDetail.pleaseRetry')
        });
      }
    }
  };
  ...
  const handleSaveAndNavigate = async () => {
    ...
    try {
      ...
      toast.success(t('articleDetail.articleUpdated'));
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          >
            ...
            {t('articleDetail.backToLibrary')}
          </Button>
          <Separator orientation="vertical" className="h-6" />
          <div className="flex items-center gap-2">
            <FileText className="h-5 w-5 text-muted-foreground" />
            <span className="text-sm text-muted-foreground">{t('articleDetail.articleType')}</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-blue-900 dark:text-blue-100">
                      {t('articleDetail.viewingVersion', { version: track.version_number })}
                    </p>
                    <p className="text-sm text-blue-700 dark:text-blue-300">
                      {t('articleDetail.olderVersionNote')}
                    </p>
                  </div>
                  <Button
                    variant="outline"
                ...
                <div className="flex-1">
                  {isEditMode ? (
                    <Input
                      ...
                      placeholder={t('articleDetail.articleTitlePlaceholder')}
                    />
                  ) : (
                  ...
                  <div className="flex items-center gap-2 mt-2 flex-wrap">
                    ...
                    {(isEditMode ? editFormData.show_in_knowledge_base : ((track.tags || []).includes('system:show_in_knowledge_base') || track.show_in_knowledge_base)) && (
                      <Badge variant="outline" className="bg-slate-100 text-slate-600 border-slate-300 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700">
                        ...
                        {t('articleDetail.inKnowledgeBase')}
                      </Badge>
                    )}
                    {/* Tags */}
                    {isEditMode ? (
            ...
            isFormDataLoaded ? (
              ...
              <Card>
                <CardContent className="p-8 text-center">
                  <p className="text-muted-foreground">{t('articleDetail.loadingEditor')}</p>
                </CardContent>
              </Card>
            )
          ) : (
            ...
            <CardHeader>
                ...
                {isEditMode && (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      title={t('articleDetail.generateKeyFactsAi')}
                    >
              ...
              {isEditMode && isGeneratingKeyFacts && (
                <p className="text-xs text-muted-foreground mt-2 flex items-center gap-2">
                  ...
                  {t('articleDetail.aiAnalyzingFacts')}
                </p>
              )}
            </CardHeader>
            <CardContent>
              {isEditMode ? (
                <div className="space-y-2">
                      ...
                      <div key={index} className="space-y-2">
                        <div className="flex items-center gap-2">
                          <Input
                            ...
                            placeholder={t('articleDetail.keyFactPlaceholder')}
                            className="flex-1"
                          />
                          <Button
                            variant="ghost"
                  ...
                  {(!editFormData.learning_objectives || editFormData.learning_objectives.length === 0) && (
                    <p className="text-sm text-muted-foreground">{t('articleDetail.noKeyFactsYet')}</p>
                  )}
                </div>
              ) : (
        ...
        <div className="space-y-6">
          ...
          {(!isSystemContent || isSuperAdminAuthenticated) && (
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">{t('articleDetail.publishingStatus')}</CardTitle>
              </CardHeader>
                  ...
                  <Badge
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        toast.success(newStatus === 'published' ? t('articleDetail.trackPublished') : t('articleDetail.trackMovedToDrafts'));
                        await onUpdate();
                      } catch (error: any) {
                        console.error('Error updating status:', error);
                        toast.error(t('articleDetail.failedToUpdateStatus'));
                      }
                    }}
                  >
                    {track.status === 'published' ? t('common.published') : t('common.draft')}
                  </Badge>
                </div>
                <p className="text-xs text-muted-foreground">
                  {track.status === 'published' ? t('articleDetail.clickToDraft') : t('articleDetail.clickToPublish')}
                </p>
              </CardContent>
            </Card>
          )}
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label htmlFor="show-in-kb" className="text-base">{t('articleDetail.showInKb')}</Label>
                    <p className="text-xs text-muted-foreground">
                      {t('articleDetail.availableInKb')}
                    </p>
                  </div>
                  <Switch
                    id="show-in-kb"
                ...
                {(isEditMode ? editFormData.show_in_knowledge_base : ((track.tags || []).includes('system:show_in_knowledge_base') || track.show_in_knowledge_base)) && (
                  <div className="pt-2">
                     ...
                     >
                       ...
                       {t('articleDetail.manageKbTags')}
                     </Button>

                     {/* Selected KB Tags Display */}
                     <div>
                       <p className="text-xs font-medium mb-2 text-muted-foreground">{t('articleDetail.selectedCategories')}</p>
                       <div className="flex flex-wrap gap-2">
                         ...
                         {(isEditMode ? editFormData.tags : track.tags || [])
                           ...
                           <span className="text-xs text-muted-foreground italic">{t('articleDetail.noCategoriesSelected')}</span>
                         )}
                       </div>
                     </div>

                     <p className="text-xs text-muted-foreground mt-2">
                       {t('articleDetail.selectKbCategoryHint')}
                     </p>
                  </div>
                )}
              </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                ...
                {t('articleDetail.contentScope')}
              </CardTitle>
              <p className="text-xs text-muted-foreground">
                {t('articleDetail.contentScopeHint')}
              </p>
            </CardHeader>
            <CardContent className="space-y-2">
              {track.scope ? (
                ...
                <p className="text-sm text-muted-foreground">{t('articleDetail.noScopeSet')}</p>
              )}
              <Button variant="outline" size="sm" onClick={() => setIsScopeModalOpen(true)}>
                {track.scope ? t('articleDetail.editScope') : t('articleDetail.setScope')}
              </Button>
            </CardContent>
          </Card>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base flex items-center gap-2">
                  ...
                  {t('articleDetail.superAdminSettings')}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label htmlFor="system-content" className="text-sm font-medium">{t('articleDetail.systemTemplate')}</Label>
                    <p className="text-xs text-muted-foreground">
                      {t('articleDetail.markAsTrikeLibrary')}
                    </p>
                  </div>
                  <Switch
                    id="system-content"
                    ...
                    onCheckedChange={(checked) => {
                      if (isEditMode) {
                        ...
                        crud.updateTrack({ id: track.id, is_system_content: checked })
                          .then(() => {
                            toast.success(checked ? t('articleDetail.markedAsSystemContent') : t('articleDetail.removedFromTrikeLibrary'));
                            onUpdate();
                          });
                      }
                    }}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">{t('articleDetail.quickStats')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-muted-foreground">{t('articleDetail.readingTime')}</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-muted-foreground">{t('articleDetail.views')}</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-muted-foreground">{t('articleDetail.likes')}</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-muted-foreground">{t('articleDetail.completionRate')}</p>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">{t('articleDetail.metadata')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              {/* Header Image */}
              {isEditMode && (
                <div className="space-y-2">
                  ...
                  {editFormData.thumbnail_url && getEffectiveThumbnailUrl(editFormData.thumbnail_url) !== DEFAULT_THUMBNAIL_URL ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            toast.success(t('articleDetail.headerImageUploaded'));
                          } catch (error: any) {
                            console.error('Error uploading image:', error);
                            toast.error(error.message || t('articleDetail.failedToUploadImage'));
                          } finally {
                      ...
                      >
                        <Upload className="h-4 w-4 mr-2" />
                        {isUploading ? t('articleDetail.uploading') : t('articleDetail.uploadHeaderImage')}
                      </Button>
                    </div>
                  )}
                  <p className="text-xs text-muted-foreground">
                    {t('articleDetail.thumbnailHint')}
                  </p>
                </div>
              )}
                ...
                <>
                  ...
                  >
                    <span className="text-sm text-muted-foreground flex items-center gap-2 shrink-0">
                      ...
                      {t('articleDetail.sourceDocument')}
                    </span>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="relative">
                  ...
                  >
                    <Upload className="h-4 w-4 mr-2" />
                    {isUploading ? t('articleDetail.uploading') : t('articleDetail.uploadAttachment')}
                  </Button>
                </div>
                <div className="space-y-2 mt-4">
                  ...
                  {attachments.length === 0 && (
                    <p className="text-sm text-muted-foreground text-center py-4">{t('articleDetail.noAttachments')}</p>
                  )}
                </div>
              </CardContent>
            </Card>
                    ...
                    <div 
                      ...
                      onClick={() => {
                        ...
                        if (!attachment.url || attachment.url === '#' || attachment.url === '') {
                          ...
                          toast.error(t('articleDetail.attachmentUrlMissing'));
                          return;
                        }
        ...
        onTagsChange={async (tags) => {
          if (isEditMode) {
            ...
            try {
              ...
              toast.success(t('articleDetail.kbCategoriesUpdated'));
              onUpdate(); // Refresh track data
            } catch (error: any) {
              console.error('Error updating tags:', error);
              toast.error(t('articleDetail.failedToUpdateKbCategories'), {
                description: error.message || t('articleDetail.pleaseRetry')
              });
            }
          }
      ...
      <VersionDecisionModal
        ...
        onVersionCreated={async (newTrackId, strategy) => {
          ...
          toast.success(t('articleDetail.versionCreated', { version: (track.version_number || 1) + 1, strategy }));
